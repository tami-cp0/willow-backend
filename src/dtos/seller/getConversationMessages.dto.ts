...
class GetConversationMessagesDto {
...
async function validateGetConversationMessagesDto(req: Request): Promise<void> {
  if (req.user.id !== req.params.userId) {
    throw new ErrorHandler(403, 'Access denied');
  }

  const dtoInstance = plainToInstance(GetConversationMessagesDto, req.params);
...
export default validateGetConversationMessagesDto;

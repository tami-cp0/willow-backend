...
async function validateGetConversationsDto(req: Request): Promise<void> {
	if (req.user.id !== req.params.userId) {
		throw new ErrorHandler(403, 'Access denied');
	}

	const dtoInstance = plainToInstance(GetConversationsDto, req.params);
...
export default validateGetConversationsDto;

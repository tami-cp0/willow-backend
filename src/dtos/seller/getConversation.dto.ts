...
async function validateGetConversationDto(req: Request): Promise<void> {
	if (req.user.id !== req.params.userId) {
		throw new ErrorHandler(403, 'Access denied');
	}

	const dtoInstance = plainToInstance(GetConversationDto, req.params);
...
export default validateGetConversationDto;

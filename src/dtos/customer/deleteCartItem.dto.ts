...
async function validateDeleteCartItemDto(req: Request): Promise<void> {
    if (req.user.id !== req.params.userId) {
        throw new ErrorHandler(403, 'Access denied');
      }

  const dtoInstance = plainToInstance(DeleteCartItemDto, req.params);
...
export default validateDeleteCartItemDto;

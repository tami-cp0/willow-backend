...
async function validateGetCustomerDto(req: Request): Promise<void> {
    if (req.user.id !== req.params.userId) {
        throw new ErrorHandler(403, 'Access denied');
    }

  const dtoInstance = plainToInstance(GetCustomerDto, req.params);
...
export default validateGetCustomerDto;
